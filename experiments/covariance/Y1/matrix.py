...
def main(tag, folder):
    ...
    data_folder = str(paths.data)
    covariance_folder = str(paths.covariance_results(tag).parent)
    paths.covariance_results(tag).mkdir(parents=True, exist_ok=True)

    # Source
    source = numpy.load(
        os.path.join(data_folder, f"{tag}/lsst_source_bins.npy"), allow_pickle=True
    ).item()
    ...
    for m in range(source_bin_size):
        source_psi_grid[m, :] = numpy.interp(
            x=z_grid, xp=source_redshift, fp=source["bins"][m]
        )
    source_psi_grid = (
        source_psi_grid
    ...
    for m in range(source_bin_size):
        table_source[f"n_{m + 1}(z)"] = source_psi_grid[m, :]
    table_source.write(
        os.path.join(covariance_folder, f"{tag}/SOURCE.ascii"),
        overwrite=True,
        format="ascii",
    )
    ...
    # Lens
    lens = numpy.load(
        os.path.join(data_folder, f"{tag}/lsst_lens_bins.npy"), allow_pickle=True
    ).item()
    ...
    for m in range(lens_bin_size):
        lens_psi_grid[m, :] = numpy.interp(
            x=z_grid, xp=lens_redshift, fp=lens["bins"][m]
        )
    lens_psi_grid = (
    ...
    for m in range(lens_bin_size):
        table_lens[f"n_{m + 1}(z)"] = lens_psi_grid[m, :]
    table_lens.write(
        os.path.join(covariance_folder, f"{tag}/LENS.ascii"),
        overwrite=True,
        format="ascii",
    )
    ...
    for m in range(source_bin_size):
        table_alignment[f"A_{m + 1}(z)"] = alignment_bias
    table_alignment.write(
        os.path.join(covariance_folder, f"{tag}/ALIGNMENT.ascii"),
        overwrite=True,
        format="ascii",
    )
    ...
    for m in range(lens_bin_size):
        table_galaxy[f"b_{m + 1}(z)"] = galaxy_bias
    table_galaxy.write(
        os.path.join(covariance_folder, f"{tag}/GALAXY.ascii"),
        overwrite=True,
        format="ascii",
    )
    ...
    for m in range(lens_bin_size):
        table_magnification[f"m_{m + 1}(z)"] = magnification_bias[m] * numpy.ones(
            grid_size + 1
        )
    table_magnification.write(
        os.path.join(covariance_folder, f"{tag}/MAGNIFICATION.ascii"),
        overwrite=True,
        format="ascii",
    )
    ...
    cosmology = pyccl.Cosmology(
        h=float(cosmology_info["H"]),
        w0=float(cosmology_info["W0"]),
        wa=float(cosmology_info["WA"]),
        n_s=float(cosmology_info["NS"]),
        A_s=float(cosmology_info["AS"]),
        m_nu=float(cosmology_info["M_NU"]),
        Neff=float(cosmology_info["N_EFF"]),
        Omega_b=float(cosmology_info["OMEGA_B"]),
        Omega_k=float(cosmology_info["OMEGA_K"]),
        Omega_c=float(cosmology_info["OMEGA_CDM"]),
        mass_split="single",
    ...
    # Cell EE
    ell = numpy.zeros(
        (source_bin_size, source_bin_size, ell_size + 1), dtype=numpy.float32
    )
    index1 = numpy.zeros(
        (source_bin_size, source_bin_size, ell_size + 1), dtype=numpy.int32
    )
    index2 = numpy.zeros(
        (source_bin_size, source_bin_size, ell_size + 1), dtype=numpy.int32
    )
    ...
    for i, j in product(range(source_bin_size), range(source_bin_size)):
        tracer1 = pyccl.tracers.WeakLensingTracer(
            cosmo=cosmology,
            dndz=[z_grid, source_psi_grid[i, :]],
            has_shear=True,
            ia_bias=[z_grid, alignment_bias],
            use_A_ia=False,
            n_samples=grid_size + 1,
        )
        tracer2 = pyccl.tracers.WeakLensingTracer(
            cosmo=cosmology,
            dndz=[z_grid, source_psi_grid[j, :]],
            has_shear=True,
            ia_bias=[z_grid, alignment_bias],
            use_A_ia=False,
            n_samples=grid_size + 1,
        )
        value[i, j, :] = pyccl.cells.angular_cl(
            cosmo=cosmology,
            tracer1=tracer1,
            tracer2=tracer2,
            ell=ell_grid,
    ...
    table_cell.write(
        os.path.join(covariance_folder, f"{tag}/Cell_kappakappa.ascii"),
        overwrite=True,
        format="ascii",
    )
    ...
    # Cell TE
    ell = numpy.zeros(
        (lens_bin_size, source_bin_size, ell_size + 1), dtype=numpy.float32
    )
    index1 = numpy.zeros(
        (lens_bin_size, source_bin_size, ell_size + 1), dtype=numpy.int32
    )
    index2 = numpy.zeros(
        (lens_bin_size, source_bin_size, ell_size + 1), dtype=numpy.int32
    )
    ...
    for i, j in product(range(lens_bin_size), range(source_bin_size)):
        tracer1 = pyccl.tracers.NumberCountsTracer(
            cosmo=cosmology,
            dndz=[z_grid, lens_psi_grid[i, :]],
            bias=[z_grid, galaxy_bias],
            mag_bias=[z_grid, magnification_bias[i] * numpy.ones(grid_size + 1)],
            has_rsd=False,
            n_samples=grid_size + 1,
        )
        tracer2 = pyccl.tracers.WeakLensingTracer(
    ...
    table_cell.write(
        os.path.join(covariance_folder, f"{tag}/Cell_gkappa.ascii"),
        overwrite=True,
        format="ascii",
    )
    ...
    index1 = numpy.zeros(
        (lens_bin_size, lens_bin_size, ell_size + 1), dtype=numpy.int32
    )
    index2 = numpy.zeros(
        (lens_bin_size, lens_bin_size, ell_size + 1), dtype=numpy.int32
    )
    value = numpy.zeros(
        (lens_bin_size, lens_bin_size, ell_size + 1), dtype=numpy.float32
    )
    ...
    for i, j in product(range(lens_bin_size), range(lens_bin_size)):
        ...
        tracer2 = pyccl.tracers.NumberCountsTracer(
            cosmo=cosmology,
            dndz=[z_grid, lens_psi_grid[j, :]],
            bias=[z_grid, galaxy_bias],
            mag_bias=[z_grid, magnification_bias[j] * numpy.ones(grid_size + 1)],
            has_rsd=False,
            n_samples=grid_size + 1,
        )
        value[i, j, :] = pyccl.cells.angular_cl(
    ...
    table_cell.write(
        os.path.join(covariance_folder, f"{tag}/Cell_gg.ascii"),
        overwrite=True,
        format="ascii",
    )
