...
def _load_timing(folder, suffix, configuration, number):
    """Load a canonical timing file, falling back to its legacy label."""

    for label in (configuration.value, configuration.legacy_value):
        candidate = folder / f'T_{label}_{number}{suffix}.txt'
        if candidate.exists():
            return numpy.loadtxt(candidate)
    raise FileNotFoundError(f'No timing file found in {folder} for {configuration.value}')
...
def main(tag, path, label, folder, number):
    ...
    figure.savefig(plot_folder / 'benchmark_{}_{}.pdf'.format(label, number), bbox_inches='tight', dpi=512)
    pyplot.close(figure)
