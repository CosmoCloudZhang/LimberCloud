...
def main(
    ...
    # Magnification
    magnification_bias = ccl_magnification_bias(
    ...
    activity = component_activity(
        alignment_bias, magnification_response_q(magnification_bias)
    )
    response_q = magnification_response_q(magnification_bias)
    ...
    for index in range(-1, int(count_list.max()) if count_list.size else 0):
        ...
        c_ccl_te = numpy.zeros((lens_bin_size, source_bin_size, ell_size + 1))
        for bin_index1, bin_index2 in product(
            range(lens_bin_size), range(source_bin_size)
        ):
            tracer1 = pyccl.tracers.NumberCountsTracer(
                cosmo=cosmology,
                dndz=[z_grid, lens_psi_grid[bin_index1, :]],
                bias=[z_grid, galaxy_bias],
                mag_bias=(
                    [
                        z_grid,
                        magnification_bias[bin_index1] * numpy.ones(grid_size + 1),
                    ]
                    if response_q[bin_index1] != 0.0
                    else None
                ),
                has_rsd=False,
            ...
            c_ccl_te[bin_index1, bin_index2, :] = pyccl.cells.angular_cl(
                cosmo=cosmology,
                tracer1=tracer1,
                tracer2=tracer2,
                ell=ell_data,
        ...
        c_ccl_tt = numpy.zeros((lens_bin_size, lens_bin_size, ell_size + 1))
        for bin_index1, bin_index2 in product(
            range(lens_bin_size), range(lens_bin_size)
        ):
            tracer1 = pyccl.tracers.NumberCountsTracer(
                cosmo=cosmology,
                dndz=[z_grid, lens_psi_grid[bin_index1, :]],
            ...
            tracer2 = pyccl.tracers.NumberCountsTracer(
                cosmo=cosmology,
                dndz=[z_grid, lens_psi_grid[bin_index2, :]],
                bias=[z_grid, galaxy_bias],
                mag_bias=(
                    [
                        z_grid,
                        magnification_bias[bin_index2] * numpy.ones(grid_size + 1),
                    ]
                    if response_q[bin_index2] != 0.0
                    else None
                ),
                has_rsd=False,
                n_samples=grid_size + 1,
            )
            c_ccl_tt[bin_index1, bin_index2, :] = pyccl.cells.angular_cl(
                cosmo=cosmology,
                tracer1=tracer1,
                tracer2=tracer2,
                ell=ell_data,
