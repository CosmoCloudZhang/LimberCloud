...
def _series_terms(a):
    return numpy.where(a <= 0.0625, 16, numpy.where(a <= 0.25, 32, numpy.where(a <= 0.5, 64, 160)))
...
def _density_series(a, power1, power2, redshift1, redshift2, source_rising, density_rising):
    ...
    def step(m, total):
        ...
        return total + a**m*value
    total = lax.fori_loop(0, _series_terms(a), step, total)
    return a**3*total
...
def ns1(chi1, chi2, power1, power2, redshift1, redshift2):
    ...
    def ordinary(_):
        ...
        return lax.cond(a <= 0.75, series, closed, None)
    return lax.cond(chi1 == 0.0, observer, ordinary, None)
...
def ss1(chi1, chi2, power1, power2, redshift1, redshift2):
    ...
    def ordinary(_):
        def series(_):
            return chi2**3*_lensing_series(a, power1, power2, redshift1, redshift2, {1:0,11:1,12:2}[1])
        def closed(_):
...
def ss11(chi1, chi2, power1, power2, redshift1, redshift2):
    ...
    def ordinary(_):
        def series(_):
            return chi2**3*_lensing_series(a, power1, power2, redshift1, redshift2, {1:0,11:1,12:2}[11])
        def closed(_):
...
def ss12(chi1, chi2, power1, power2, redshift1, redshift2):
    ...
    def ordinary(_):
        def series(_):
            return chi2**3*_lensing_series(a, power1, power2, redshift1, redshift2, {1:0,11:1,12:2}[12])
        def closed(_):
