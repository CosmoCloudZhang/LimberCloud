...
def _element3_nonzero(chi1, chi2, chi3, chi4, chi5, power1, power2, redshift1, redshift2):
    ...
    def false_branch(_):
        formula = (a * ( - 12 * b * p * z + 2 * a ** 2 * b * ( - 6 + 3 * p + 3 * z - 2 * p * z) + 6 * a * b * (2 * p + 2 * z - p * z) + a ** 3 * c * (6 - 4 * p - 4 * z + 3 * p * z)) - 12 * b * (a - p) * (a - z) * jnp.log(chi1 / chi2)) / (12 * a ** 3)
        return formula
...
12 * (-chi1 / chi2) * b * (a - p) * (a - z) * jnp.log(chi1 / chi2)) / (12 * a ** 3)
        return formula
    formula = lax.cond(chi1 == 0.0, true_branch, false_branch, operand=None)
...
a ** 4 * (6 - 4 * p - 4 * z + 3 * p * z)) * jnp.log(chi1 / chi2) + a ** 5 * (1 + b) * (6 - 4 * z + p * ( - 4 + 3 * z)) * jnp.log(1 + b))))
        return formula
    formula = lax.cond(chi1 == 0.0, true_branch, false_branch, operand=None)
...
2 * a * (5 * z + p * (5 + 8 * z + 3 * b * z))) * jnp.log(chi1 / chi2) + a * ( - 540 * p * z + a ** 4 * ( - 600 + p * (130 - 53 * z) + 130 * z) + 30 * a * (20 * z + p * (20 + 41 * z + 12 * b * z)) +
...
def element4(chi1, chi2, chi3, chi4, chi5, power1, power2, redshift1, redshift2):
    ...
    def corrected(_):
        left = _element4_nonzero(chi1, chi2, chi3, chi4, chi5, jnp.ones_like(power1), jnp.ones_like(power1), redshift1, redshift2) - _element4_nonzero(chi1, chi2, chi3, chi4, chi5, jnp.zeros_like(power1), jnp.ones_like(power2), redshift1, redshift2)
        left = jnp.where(chi1 == 0.0, 0.0, left)
        return jnp.where(zero, power1*left, regular)
    return lax.cond(jnp.any(zero), corrected, lambda _: regular, None)
...
def element6(chi1, chi2, chi3, power1, power2, redshift1, redshift2):
    ...
    def corrected(_):
        left = _element6_nonzero(chi1, chi2, chi3, jnp.ones_like(power1), jnp.ones_like(power1), redshift1, redshift2) - _element6_nonzero(chi1, chi2, chi3, jnp.zeros_like(power1), jnp.ones_like(power2), redshift1, redshift2)
        left = jnp.where(chi1 == 0.0, 0.0, left)
        return jnp.where(zero, power1*left, regular)
    return lax.cond(jnp.any(zero), corrected, lambda _: regular, None)
...
def element7(chi1, chi2, chi3, chi4, power1, power2, redshift1, redshift2):
    ...
    def corrected(_):
        left = _element7_nonzero(chi1, chi2, chi3, chi4, jnp.ones_like(power1), jnp.ones_like(power1), redshift1, redshift2) - _element7_nonzero(chi1, chi2, chi3, chi4, jnp.zeros_like(power1), jnp.ones_like(power2), redshift1, redshift2)
        left = jnp.where(chi1 == 0.0, 0.0, left)
        return jnp.where(zero, power1*left, regular)
    return lax.cond(jnp.any(zero), corrected, lambda _: regular, None)
...
def element8(chi1, chi2, chi3, chi4, power1, power2, redshift1, redshift2):
    ...
    def corrected(_):
        left = _element8_nonzero(chi1, chi2, chi3, chi4, jnp.ones_like(power1), jnp.ones_like(power1), redshift1, redshift2) - _element8_nonzero(chi1, chi2, chi3, chi4, jnp.zeros_like(power1), jnp.ones_like(power2), redshift1, redshift2)
        left = jnp.where(chi1 == 0.0, 0.0, left)
        return jnp.where(zero, power1*left, regular)
    return lax.cond(jnp.any(zero), corrected, lambda _: regular, None)
...
def coefficient(chi_grid, power_grid, redshift_grid):
    ...
    def accumulate_step(n, coefficients):
        ...
        # The node n+1 density hat and the partly covered node n source hat both
        # live on this interval, so the terminal density row n+1 = N needs no
        # next node and is stored on every interval.
        ...
        coefficients = coefficients.at[n, n + 1, :].add(jnp.where(valid, value5, jnp.zeros_like(value5)))
        ...
        # evaluation point sits inside that hat, so both placements switch to
        # the truncated source integral H_N(x).
        value7 = lax.cond(valid, lambda _: element7(chi_grid[n], chi_grid[n + 1], chi_grid[-2], chi_grid[-1], power_grid[:, n], power_grid[:, n + 1], redshift_grid[n], redshift_grid[n + 1]), lambda _: element9(chi_grid[n], chi_grid[n + 1], power_grid[:, n], power_grid[:, n + 1], redshift_grid[n], redshift_grid[n + 1]), None)
        coefficients = coefficients.at[n, grid_size, :].add(value7)

        value8 = lax.cond(valid, lambda _: element8(chi_grid[n], chi_grid[n + 1], chi_grid[-2], chi_grid[-1], power_grid[:, n], power_grid[:, n + 1], redshift_grid[n], redshift_grid[n + 1]), lambda _: element10(chi_grid[n], chi_grid[n + 1], power_grid[:, n], power_grid[:, n + 1], redshift_grid[n], redshift_grid[n + 1]), None)
        coefficients = coefficients.at[n + 1, grid_size, :].add(value8)
