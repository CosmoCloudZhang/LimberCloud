...
# Run applications
srun -n 1 -c $SLURM_CPUS_PER_TASK python -u "${BASE_PATH}JAX/CPU/${TAG}/${LABEL}.py" --tag=$TAG --path=$BASE_PATH --label=$LABEL --folder=$BASE_FOLDER --number=$SLURM_CPUS_PER_TASK
