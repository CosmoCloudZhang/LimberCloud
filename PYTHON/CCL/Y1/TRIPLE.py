...
def main(tag, path, label, folder, number):
    ...
    # Save
    numpy.savetxt(os.path.join(python_folder, 'CCL/', tag, 'T_{}_{}.txt'.format(label, number)), time_list)
    numpy.savetxt(os.path.join(python_folder, 'CCL/', tag, 'T_{}_{}_CELL.txt'.format(label, number)), time_cell_list)
    numpy.savetxt(os.path.join(python_folder, 'CCL/', tag, 'T_{}_{}_COSMOLOGY.txt'.format(label, number)), time_cosmology_list)
