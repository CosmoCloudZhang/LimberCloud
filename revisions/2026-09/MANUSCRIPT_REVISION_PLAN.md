...
**Finalised 19 September 2026 — implementation plan, not an implemented manuscript revision.** Read alongside [COAUTHOR_COMMENT_INVENTORY.md](COAUTHOR_COMMENT_INVENTORY.md), [CODE_REVISION_PLAN.md](CODE_REVISION_PLAN.md), and [CURSOR_IMPLEMENTATION_PROMPTS.md](CURSOR_IMPLEMENTATION_PROMPTS.md). This update consolidates the author's decisions on the environment, manuscript submodule, local/Perlmutter workflow, HDF5 spectra, fiducial-plus-ensemble experiment, serial benchmark execution, absolute error plots, and the scope of interpolation/grid studies. All 102 comment IDs and their individual dispositions are retained. Existing line-linked scientific source locations refer to GitHub commit [`7d29b2f`](https://github.com/CosmoCloudZhang/LimberCloud/tree/7d29b2f0a5e4f216bd4dd9b4cd36b9ffb6a4487c), the 15 September audit baseline; historical local checkout references in supporting audits are historical evidence, not descriptions of the present checkout.
...
The local repository has since reached parent commit `b16dcdc5486eed921dda447168fe2761ff2d2514`, with `manuscript/` an optional submodule at paper commit `90d12f4f3e574a67c25944d27d7ded553e09402b` in `LimberCloudPaper`. These identities were verified on 19 September and establish the planning baseline, not the future results version. Project guides now belong under `documents/`; the review package remains at `revisions/2026-09/`. Preserve historical source links above and use the current paper repository for actual edits. The subtree conversion is complete; documentation describing it as the ongoing manuscript workflow still needs correction.
...
| --- | --- | --- |
| E0 — Environment and repository handoff | Reconcile both Git repositories, actual CFS inputs/results, the existing NERSC kernel/environment and module stack. Prepare the dedicated LimberCloud environment and a shared script/notebook interpreter on each platform, with h5py and NERSC-compatible mpi4py. | Code/paper commit identities, external-input inventory, explicit CPU/CUDA recipes and exact tested package/module records; allocated tiny-run and kernel smoke results; working environment retained until replacement passes |
...
Local Codex owns planning, review, portable maintenance and manuscript editing; the remote Cursor agent owns changes and validation that require Perlmutter modules, CFS inputs, allocated CPUs/GPUs or OneCovariance. Assign one implementation owner to a given file at a time and exchange bounded commits plus evidence reports. The manuscript can be reorganised locally before final numerical products exist, while numerical claims and publication figures remain gated by accepted remote results. The remote report must provide accessible summaries/provenance and selected publication outputs so local manuscript work does not depend on mounting CFS. Production arrays remain in the external runtime tree.

The `LimberCloudPaper` repository is the source of truth for TeX and publication figures; the parent repository pins its accepted revision. For paper edits, use a named paper branch, commit and push there first, then commit and push the updated `manuscript` gitlink in LimberCloud. A parent commit alone cannot save uncommitted paper changes. Receive the parent-pinned revision with `git submodule update --init manuscript`, then switch to the intended paper branch before editing. Inspect and preserve remote manuscript work before the first migration there. Code-only users need not initialise the submodule, and code validation must remain independent of LaTeX or paper availability.

Update `documents/manuscript-workflow.md` and README links for this arrangement. Where Overleaf synchronization is used, perform it directly from the paper repository and document the actual remote/branch setup; the parent-repository subtree push is no longer the routine workflow. Paper provenance records the code commit that generated results plus input/configuration, sample-table and run identities. The parent subsequently records the paper commit. Do not require each repository to record the other's new commit simultaneously: that creates an impossible circular hash dependency. The completed submodule conversion does not remove old manuscript objects from the parent Git history; history rewriting is outside this plan.
...
Regenerate through the declared producers and copy only validated publication outputs into `manuscript/figures/`. The live figure manifest still names old notebook directories despite the recent rename; for example its power producer says `notebooks/matter_power/` while the code now uses `notebooks/power/`. Repair these entries in the paper repository in coordination with the code owner. Record artifact IDs/hashes, effective configuration, selection and software revision in provenance metadata. Inspect at final page size: typography, clipping, legend placement, shared axes, colour consistency, displayed sample and covariance component. A successful TeX build alone is insufficient.
...
2. Complete E0 and execute E1–E5 using the staged code plan; keep failed/unmatched historical artifacts identifiable instead of overwriting their provenance. The inclusion of mpi4py/h5py does not enlarge the paper benchmark into an MPI scaling study.
...
8. Build a fresh manuscript and visually inspect the complete PDF. Only then mark comments addressed and prepare concise co-author responses with the actual revised text. Commit and push the paper revision before updating the parent's submodule pointer; keep actual publication/submission separate from local editing and validation.

## 7. Manuscript-agent handoff

Use the manuscript handoff in [CURSOR_IMPLEMENTATION_PROMPTS.md](CURSOR_IMPLEMENTATION_PROMPTS.md) with this plan and the code plan available to the assigned paper agent. The default is local Codex for paper work and remote Cursor for scientific execution; do not assign simultaneous edits to the same files. First reconcile parent and paper branch/commit identities, confirm that `manuscript/` is initialised, and preserve in-progress author changes. Make the structural/notation/source-link edits and comment ledger before accepted numerical products arrive; keep numerical claims explicitly pending rather than importing old results into the new ensemble narrative. Use fiducial ID 0, the shared sample-table ID, validated covariance/mask and saved HDF5 spectra or exported per-sample summaries supplied by the code agent for final tables and figures. Apply this update's dispositions when they supersede the 15 September audit or a tentative earlier recommendation.

After E0–E5 products pass their gates, regenerate figures, choose the optional ensemble-distribution visualization from the measured spread, update numerical prose together, verify bibliography and availability, compile and visually inspect every page. Return a paper source diff, comment-response ledger, figure/summary provenance, environment/run evidence references and a clear list of unresolved scientific choices. Report both code-result and paper identities; update the parent gitlink only after the paper commit is available remotely. Do not claim production runs, optimized grids, MPI scaling, parallel HDF5 throughput, arbitrary-function analytic integration or noisy-data reduced-chi-square acceptance that the evidence does not establish.
