...
**Updated 22 September 2026 — authoritative paper plan aligned with new Phases 1–5.** Read the [code plan](CODE_REVISION_PLAN.md), [prompt index](CURSOR_IMPLEMENTATION_PROMPTS.md), [completion checklist](COMPLETION_CHECKLIST.md), and [102-comment inventory](COAUTHOR_COMMENT_INVENTORY.md). The per-comment dispositions and original attribution below are retained; later author decisions are not additional coauthor claims. Historical source links still identify their original audited commits, not live edit locations.

The updated source review is against parent `13a3c2d`: environment/maintenance and part of the sample/artifact foundation exist, but scientific execution and acceptance remain incomplete. [The current review](reports/2026-09-22_IMPLEMENTATION_REVIEW.md) records remaining work, including the accidental removal of the manuscript gitlink in that commit. Restore the reviewed paper pin `90d12f4f3e574a67c25944d27d7ded553e09402b` in the parent without replacing the local paper repository; inspect both index and HEAD until an authorized commit records the repair. The paper remains locally owned. Remote Cursor never initializes or edits it.

The author has now fixed eta_IA=0.0, NUMERIC-only radial interpolation orders, one common 21-node evaluation grid from 20 to 2000, and 20 bandpowers from the notebook natural cubic spline of ell*C versus logell divided by linear bin width. These bandpowers are the residual and covariance vector; raw 21-node samples are reproducibility data, and centres are plotting coordinates. Remove current 101-grid or centre-evaluation instructions. Include the final NN diagonal, keep the cubic-observer 1/4 factor, and incorporate independently validated NS/SN/SS terminal cases. The separate numerical backend follows the notebook radial interpolation/quadrature comparison; it is not the matched-analytical-integrand reference.

Use [Phase 5B](prompts/PHASE_5B_LOCAL_MANUSCRIPT.md) for local execution. Editorial work and the ledger can proceed alongside remote Phases 1–4; final numerical claims require accepted Phase 4/5A exports. No new scientific result, runtime or successful manuscript revision is asserted by this planning update.

## 1. Recommended high-level revision

The [current Phase 1 review](reports/2026-09-22_PHASE1_CONTRACT_REVIEW.md)
reopens endpoint stability/observer acceptance and nuisance identity enforcement.
The current fixed 48-point terminal quadrature is a temporary implementation. The expanded Phase 1 plan targets analytic NS B09/B10 and SS B11/B12, stable observer/narrow/zero limits, and independent review plus compiled validation before terminal.py removal. Use the accepted implementation when describing the method; if that removal gate remains unmet, record the incomplete closeout and qualify any retained quadrature honestly. Exact bilinear contraction does not make finite quadrature exact. The 21 original Mathematica derivations gain faithful Jupyter editions with annotated notation corrections, and the four new cases use the same layout. Link accepted derivations and current compact regressions; a planning review, removed tests or historical report alone cannot establish endpoint acceptance.
...
Local editorial reorganisation can proceed in parallel with remote code validation. NERSC agents use the committed plans and comment inventory as their paper context; manuscript source access and a paper-repository credential are not prerequisites. Numerical conclusions and statistical interpretations must wait for the following dependencies.

| Gate | Work | Completion evidence |
| --- | --- | --- |
| E0 — Environment and repository handoff | Remote Cursor inventories the parent checkout, actual CFS inputs/results, existing NERSC kernel/environment and module stack. It implements the dedicated environment and shared script/notebook interpreter, with h5py and NERSC-compatible mpi4py. Local Codex verifies the local paper checkout separately. | Remote code commit and recorded paper pin from `git ls-tree HEAD manuscript`; local actual paper commit/branch; external-input inventory, CPU/CUDA recipes and exact tested package/module records; allocated tiny-run and kernel smoke results; working environment retained until replacement passes |
...
| E4 — Accuracy evidence | Run one explicit fiducial and 1,000 matched sampled cosmologies per adopted survey configuration; use scripts to save HDF5 spectra and notebooks to analyse selected stored data. Apply one saved mask to plots, residuals and covariance. | Validated HDF5 schema and per-sample spectra/status/identities, counts, pointwise percentiles, worst pair/band, undefined-ratio counts and a per-cosmology joint-discrepancy table; fiducial excluded from ensemble quantiles |
...
| E6 — Local paper and figure revision | Local Codex moves sections, implements individual edits below, imports accepted remote publication products and reconciles bibliography/availability. Remote Cursor owns any changes to figure producers and regenerates their outputs on NERSC. | Local paper source diff, complete comment-to-change mapping, verified transferred file hashes and traceable figure manifest |
| E7 — Local final review | Compile the whole manuscript locally and inspect every page, equation, figure and reference. | No unresolved references, clipping, legend overlap or unexplained claims; each closed comment cites a concrete change and evidence |
...
### Environment and execution scope to describe accurately

Use one dedicated LimberCloud Python environment per machine for scripts, notebook kernels and editor interpretation. Remote Cursor maintains a portable CPU recipe for reproducible CPU use and a compatible NERSC CUDA variant for GPU experiments and, after verification, CPU experiments through explicit device selection. This recipe portability does not assign code implementation to the local paper agent. Local code or test implementation requires an explicit ownership handoff; local review and smoke verification remain allowed. Do not promise an identical binary environment across macOS and Perlmutter. The code plan makes `.venv` the local reference to the selected environment, keeps `.env` for external configuration, and removes redundant environment-selection variables only after all launchers and their remote dependencies are reconciled. `PROJECT_ROOT` identifies the code checkout; `RUNTIME_ROOT` remains the external data/results tree. Source and formatting changes belong to the code plan. Mechanical cleanup preserves scientific behaviour; the approved physics and estimator corrections deliberately change affected results and require new identities and evidence.

The project environment excludes CosmoSIS and includes h5py for HDF5 spectrum storage and mpi4py for future MPI workflows. Validate mpi4py against the actual NERSC MPI installation using current NERSC instructions. Availability of mpi4py does not introduce task-based parallelism into this paper: the primary benchmark remains one task with a sequential cosmology loop, plus existing Numba threading and JAX execution. Ordinary single-writer h5py is sufficient for that storage contract; MPI-enabled h5py and concurrent file writing are not prerequisites for the paper campaign. Record the actual HDF5/MPI build capabilities without claiming parallel-I/O or MPI speed-up measurements.

Separate Python requirements from the external software needed to reproduce all source material: current figure rendering uses LaTeX, symbolic `.nb` derivations use Mathematica, and OneCovariance has its own versioned environment/dependency requirements to audit remotely. Retain the working CosmoConda installation while validating the smaller replacement. Host CPU count stays in the Slurm allocation, not in a `--number` flag or a product filename. Use the explicit sample-count controls before requesting a tiny scientific smoke run.

### Ownership and publication workflow

Local Codex owns planning, code/report review, manuscript prose/equations/references/captions/tables, publication-figure integration, the comment-response ledger and final PDF compilation/review. Remote Cursor owns code, scripts, notebooks, environment setup, scientific tests, experiment execution, summary generation and figure producers on NERSC. Assign one implementation owner to each file; local Codex does not concurrently implement code, even portable maintenance, unless the author explicitly transfers that work. Editorial changes can proceed locally before final numerical products exist. Numerical claims and publication figures remain gated by accepted remote evidence.

The `LimberCloudPaper` repository is the source of truth for TeX and publication figures; the parent repository pins its accepted revision. Keep its checkout local. The NERSC manuscript submodule is intentionally uninitialized and may have no directory: retain `.gitmodules` and the tracked gitlink, do not add `manuscript/` to `.gitignore` or remove its reference with `git rm`. Remote scientific work, imports, ordinary checks and figure export must work without manuscript source or LaTeX manuscript compilation. The remote agent reads these committed revision documents for paper context and records the parent's paper pin with `git ls-tree HEAD manuscript`; it must not assume `git -C manuscript` is usable. No paper-repository authentication is needed for this workflow.

Use this transfer contract for publication evidence:

1. Remote Cursor produces a versioned export bundle on CFS containing selected publication figures, compact CSV/JSON summaries, a run/evidence report and a manifest with relative filenames and SHA-256 hashes. Include the code result commit, effective configuration and input identities, sample-table/run IDs, covariance/mask identities, producer commands and environment records specified below. Large spectra, covariance and coefficient datasets remain on CFS and out of Git.
2. Local Codex reviews the report and requested claims, transfers the accepted bundle through the user's configured file-transfer route, verifies its manifest hashes, and preserves the CFS location plus portable artifact identities. The transfer is explicit; Git does not copy CFS datasets. A missing result or failed validation remains an open gate.
3. Only local Codex copies accepted publication figures into `manuscript/figures/`, updates its manifest, integrates summaries into tables/prose and records comment responses. If a producer change is needed, request a bounded remote code change and replacement bundle; do not edit the same producer locally. Local rendering from accepted compact products can be assigned separately, without making CFS access a prerequisite for paper editing.
4. Compile and inspect the paper locally. Commit and push in the paper repository first, then commit and push the updated parent gitlink. A parent commit alone cannot save uncommitted paper changes. The paper records the actual code-result revision; the later parent commit records the paper revision. Do not create a circular requirement for both new commits to contain each other's hashes.

On NERSC, receive parent changes with `git pull --ff-only --no-recurse-submodules`; do not run `git submodule update --init` as routine scientific setup. Locally, with clean working trees, receive the parent with `git pull --ff-only` and its pinned paper with `git submodule update --init --recursive manuscript`. Submodule update normally leaves detached HEAD; switch the local paper to the intended named branch before editing. Preserve uncommitted work before any synchronization. The author's terminal output confirmed successful NERSC submodule initialization after preserving an old directory containing build products; the later decision is to leave the paper deinitialized or absent there. The local agent has not directly verified that final remote state. Repeating the migration or deleting backup files is not an implementation step.

Follow [the manuscript workflow](../../documents/manuscript-workflow.md) for local paper commits and optional direct paper-repository Overleaf synchronization; parent-repository subtree push is historical, not the routine workflow. The submodule conversion does not remove old paper objects from the parent's Git history, and deinitializing the NERSC working directory does not erase cached Git objects. History rewriting is outside this plan.
...
| ID | Proposed edit / disposition | Required evidence |
| --- | --- | --- |
| N09 | Define rho_m,0=Omega_m rho_crit,0 and C1 units; the generator already uses comoving density at a=1. Verify agreement with the CCL raw IA coefficient. The author has resolved eta_IA=0.0; regenerate matching arrays/results and preserve the distinct amplitude and pivot values. | IA numerical validation |
...
The author has adopted eta_IA=0.0 for the current SRD-based fiducial. Remove the default/unresolved eta=0.5 execution branch and regenerate affected IA arrays and dependent products with new provenance. Keep A_IA=0.5 and z_pivot=0.5. Historical eta=0.5 results remain historical. The current scientific table, evaluator, covariance inputs and accepted figures must all match eta=0; changing a metadata label alone does not regenerate a physical result.

Describe the validated effective fiducial model behind both IA and galaxy-bias tables, including radiation/neutrino and solver conventions, not only their primary parameter values. The current generators and sample constructor differ in explicit Omega_g and CAMB kmax settings; the remote checks must establish consistency or document an intentional distinct fixed nuisance-reference model. Do not infer a numerical disagreement from settings alone, or conceal an unverified difference under a common fiducial label.
...
With the declared independent catalogue/bin-noise assumptions, use n_sr=n_arcmin^-2*(180*60/pi)^2, lens shot noise 1/n_lens and shape noise sigma_e,component^2/n_source. The stated per-component sigma_e=0.26 does not acquire another factor 1/2. Zero cross-noise requires the actual count/shape and sample-overlap assumptions. Distinguish these physical noise terms from the reconstructed noise-free signal, and quote the accepted remote unit/amplitude tests.
...
Define every accepted data element as a uniform-dell bandpower from the same 21 float64 geomspace edges over 20–2000. Every method, including CCL, evaluates those 21 nodes; a natural cubic spline of ell*C_ell against logell is integrated in each bin and divided by its linear width. There are 20 comparison/covariance elements per pair. Save the raw 21 samples for reconstruction, with geometric centres used only for display. The old CCL centre evaluations and separate 101-point covariance grid are legacy products. Pin the natural boundary condition explicitly; the first shared helper used default not-a-knot. Covariance must use identical floating-edge windows and justified physical normalization, not merely matching labels. Internal covariance quadrature may refine the canonical reconstruction without introducing another shared science grid. Raw theoretical node samples are not independent observed modes.
...
Positive-definiteness and solve checks concern the adopted complete covariance and Gaussian reference. Connected NG corrections may be signed and need not be PSD by themselves; explain component-model/window/convergence checks and their accepted sum without implying each correction must pass Cholesky. Keep separately accepted Gaussian results and pending upstream/full-covariance evidence clearly labelled; a completed spectra campaign does not certify an unavailable covariance component.

The input ordering repair must precede covariance regeneration; the output pair-index repair must precede plotting or likelihood use. A simultaneous row/column permutation preserves eigenvalues and cannot repair an indefinite already assembled matrix. The two VAECloud artifacts inspected in the earlier audit both failed Cholesky; they were not the successful corrected reference recalled by the user and are not assertions about files in this current checkout. Retrieve the exact successful configuration/revision before treating it as a regression fixture.
...
Report chi in Mpc, k in Mpc^-1, P in Mpc^3 and the actual solver/extrapolation range. The accepted implementation must call the power provider only at positive finite chi and set its observer ordinate from the declared limit; replacing infinite k with the largest float is not a physical limit prescription.
...
NUMERIC preserves its separate notebook radial interp1d boundary behavior; its radial cubic is not the angular natural spline. State its accepted knot-split quadrature and observer integrability checks. For instance, power proportional to chi with both density weights nonzero at the observer produces a divergent NN integral of 1/chi; a finite fixed-order Gauss result cannot validate that case. This observer restriction is distinct from the unrestricted far-endpoint coefficient corrected in the analytical basis.
...
**Resolved terminal-node requirement:** the full nodal basis includes the final density node. Correct the NN final diagonal and the related NS/SN/SS terminal cases in remote Phase 1; update the mathematical index ranges and the ordinary versus additional boundary-case catalog from that evidence. Retain observer factors 1/12, 1/12 and 1/4. A last-node basis has support across the whole final interval, even though changing a function at a single integration endpoint has measure zero. Do not replace the missing cases with a forced-zero endpoint assumption.
...
The expression at source label `eq:4.9b` (displayed Eq. 4.10) contains n+2 and needs a special case on the final interval n=N_z-1; the generic last-node expression also assumes a full supporting interval. The text saying only i=n has a partial interval is false: i=n+1 contains one too. Derive and implement the truncated endpoint expressions for unrestricted final nodal coefficients, matching the accepted NN/NS/SN/SS support tests. Do not impose a zero-endpoint distribution to conceal missing basis terms or treat this as notation cleanup.

Independent ordinary-interval checks and agreement between Numba/JAX ports are useful but insufficient for endpoints. The 15 September audit interpreted the first NN interval using linear power; the fresh review found an explicit cubic observer-interval convention in the power notebook. The NN values 1/12, 1/12 and 1/4 agree with that cubic convention, so do not change 1/4 to 1/2 based on the old premise. Reconcile this convention with every coefficient family and the derivation text; include and independently validate the final NN diagonal and related mixed/lensing terminal cases. Twenty-one Mathematica derivation notebooks already exist: preserve them and publish faithful same-basename Jupyter editions, including all scientific content and clearly identified cached outputs. Annotate verified notation corrections and add NS B09/B10 and SS B11/B12 with matched validation notebooks; do not claim a fresh Wolfram run from transcription. Observer-limit tests must implement the actual power/support convention and verify integral finiteness; reject divergent fixtures and test nonzero far-endpoint density separately.
...
| ID | Proposed edit / disposition | Required evidence |
| --- | --- | --- |
| JZ09 | Reconcile plotted arrays and claims. Show comparable end-to-end times. Use legend label CCL; explain in the caption that its repeated stage-panel curve is the end-to-end reference, not a measured CCL stage. | Timing validation |
...
Report at least: cold initialisation/JIT; warm full prediction (cosmology+coefficient+contraction+required transfers); warm coefficient plus contraction with a supplied cosmology/power state; and fixed-basis contraction for a changed redshift distribution. The latter two are a separate bounded benchmark piloted in Phase 3 and accepted in Phase 4, not extra cosmologies in the 1,000-row campaign. Preserve distribution fixtures, cached-state identities, repetition counts and equivalent 20-band outputs. Compare complete distribution-update work against CCL reusing cosmology/power while rebuilding affected tracers; report isolated contraction separately. State how many ell outputs and bin pairs are produced, output precision and hardware. A complete likelihood also includes its covariance/data operations, so a spectrum timer should not automatically be labelled likelihood time.
...
Figure notebooks consume the accepted saved HDF5 spectra/timings and metadata; they do not rerun the 1,001-case calculations. The saved-spectrum name and schema follow the code plan, including explicit NUMERIC interpolation identity: for example `Spectra_Triple_EE.h5` and `Spectra_Triple_LINEAR_EE.h5` within their method/device/survey directories, with accepted generations retained by the transaction/archive layer. HDF5 `.h5` replaces the earlier proposed spectrum `.npz` products; the separate canonical `Cosmologies.npz` table can retain its format. Host CPU count is Slurm runtime metadata, not a filename token and not a GPU count. The performance figure plots CCL, Numba and JAX only. Use the code plan's separate Fiducial/Cosmology timing names and recorded sample-count columns, adding interpolation identity only on NUMERIC products. Keep legend label `CCL`; explain its repeated end-to-end stage-panel reference in the caption. Distinguish NUMERIC integration timing from isolated analytic contraction timing and avoid promising a production run duration before pilot measurements.

Include a compact reproducibility statement in the performance section, backed by complete machine-readable run metadata: code commit and dirty state; Python and scientific-package versions; effective CPU/thread settings and JAX device/precision; GPU/driver/CUDA versions where used; NERSC modules, compiler/MPI library identity, and h5py/HDF5 versions and MPI capability. State the single-task workload separately from software that is merely available in the environment. I/O and checkpoint work remain outside compute-stage timing, with total wall time recorded independently. If a run is resumed, distinguish the accumulated compute measurements across segments from a continuous uninterrupted job-time measurement.

Current JAX calls already synchronize results; preserve those barriers and separate explicit pre-sample warm-up/compilation. Any recompilation or lazy preparation inside a timed sampled call remains charged and diagnosed, never retrospectively subtracted. Correct sampled-cosmology prefactors, magnification assembly and ell estimators before reusing any runtime result for a physical comparison. Include an efficient CCL fixed-cosmology/tracer-caching comparison for nuisance updates. Do not compare an isolated GPU contraction to the full CPU pipeline and describe the ratio as an end-to-end algorithm speed-up.
...
Include [the public LimberCloud repository](https://github.com/CosmoCloudZhang/LimberCloud), the actual commit/release used for the results, access instructions for example inputs/configuration and the shared sample table, and the distinction between tracked code/publication figures and external runtime data. Explain the optional manuscript submodule in the reproducibility documentation, without implying that a standard code installation requires the paper. State the access status of `LimberCloudPaper` only after checking it. The review commit is not automatically the future results commit. Supply environment recipes plus exact tested environment/module records and distinguish locally reproducible CPU work from the reported Perlmutter CPU/GPU benchmark. Check acknowledgments against the hardware actually used for the reported timings; the existing draft credits Google Cloud while the current run plan targets Perlmutter. Preserve collaborator funding and authorship unless separately instructed.
...
Remote Cursor regenerates figures through the declared producers into the CFS export bundle; local Codex verifies the transferred hashes and copies only accepted publication outputs into `manuscript/figures/`. The remote producer must not require that directory to exist. The live figure manifest still names old notebook directories despite the recent rename; for example its power producer says `notebooks/matter_power/` while the code now uses `notebooks/power/`. Local Codex repairs these entries in the paper repository using the remote code owner's verified producer paths and artifact report. Record artifact IDs/hashes, effective configuration, selection and software revision in provenance metadata. Inspect at final page size: typography, clipping, legend placement, shared axes, colour consistency, displayed sample and covariance component. A successful TeX build alone is insufficient.

Keep the spectra and error notebooks restricted to reading the requested HDF5 sample/probe slices, loading or computing bounded summaries and rendering. Read the fiducial by ID 0 without loading all 1,001 spectra into memory; compute ensemble summaries in bounded chunks where needed. Derivation and targeted kernel/power diagnostic notebooks retain their explicitly scoped validation role. Bound canvas size at the intended manuscript dimensions; moving computation to scripts alone does not remove the memory cost of oversized plots. Save raw summary tables separately from display transforms so log floors, omitted markers and drawing order cannot change reported statistics.

### Minimum figure and table provenance

Local Codex extends `manuscript/figures/manifest.toml` or a linked versioned sidecar with the following fields; use one consistent schema and validate required values before accepting outputs. Producer paths are relative to the LimberCloud code checkout even though the manifest lives in the paper repository. Runtime paths are external locators, not portable artifact identities.

| Provenance group | Required content |
| --- | --- |
| Producer and output | Code repository and results-producing commit/dirty state, producer path, command or notebook parameters, output filename and content hash, manifest/schema version |
| Spectrum inputs | Run ID(s), HDF5 artifact hash(es) and schema version, backend/device and NUMERIC interpolation order, survey and Single/Double/Triple workload, sample-table ID/hash, explicit fiducial ID and sampled-ID set/status counts |
| Physical and numerical definition | Input/configuration hashes, units, precision, grids, labelled ell/pair axes, observable components, interpolation and endpoint convention, quadrature/reference tolerances, ell estimator/window identity |
| Statistical definition | Covariance identity/component/hash and fixed-fiducial convention, selection mask/hash and vector ordering, residual formula, per-point valid counts and undefined-reference counts, quantile convention and exclusion of fiducial, discrepancy-summary artifact/hash |
| Execution environment | Reference to exact environment and NERSC module records, hardware/device and thread/resource settings, h5py/HDF5 and MPI library identity/capability, timing boundaries and segmentation status when relevant |
| Presentation | Caption/figure label, plotted sample or ensemble selection, display-only zero/floor/masking rules, source summary-table hash and final-size visual check status |

The HDF5 reader must reject incompatible sample tables, configurations, axes or unfinished sample records before generating scientific summaries. Do not silently join by file position, treat a successfully opened HDF5 file as evidence of a completed run, or alter statistics through plotting transforms. The storage/checkpoint consistency contract is owned by the code plan. Track compact manifests, summaries and validated publication figures as appropriate; retain production arrays in CFS and give their access/export instructions separately. Preserve the exact code-result identity when later documentation or parent gitlink commits advance the checkout.
...
| Current scientific configuration differs from generated/benchmarked quantities | Current 3.1–3.2 and experiment scripts | Set effective model contract; fix active-cosmology prefactors/magnification assembly; enforce the adopted eta_IA=0.0 and regenerate dependent evidence |
...
1. Maintain the implementation ledger locally in the paper repository: comment ID, changed source/figure, response text, evidence artifact and unresolved question if any. Multiple IDs may link to one change.
2. Have remote Cursor complete the remote E0 inventory and execute E1–E5 using the staged code plan; local Codex verifies the local paper identity and reviews remote evidence; keep failed/unmatched historical artifacts identifiable instead of overwriting their provenance. The inclusion of mpi4py/h5py does not enlarge the paper benchmark into an MPI scaling study.
3. Make the structural move locally, separately from scientific edits where practical. Preserve labels first; avoid mechanical line-number-based edits to the older marked PDF.
...
5. Transfer and hash-check the accepted CFS export bundle, integrate its figures locally and recompute all numerical prose from its summaries; make abstract, introduction, captions, discussion and conclusion agree.
...
7. Verify bibliography, code/data availability, actual benchmark hardware acknowledgments and collaborator-supplied front matter. Check the code/environment/run/figure identities and update `documents/` workflow instructions for the optional paper submodule and direct paper-repository Overleaf synchronization.
8. Build a fresh manuscript locally and visually inspect the complete PDF. Only then mark comments addressed and prepare concise co-author responses with the actual revised text. If Git publication is explicitly requested, commit and push the reviewed paper revision before updating/publishing the parent pointer; do not automatically publish as part of editing and validation.

## 7. Local manuscript-agent handoff

Use **[Phase 5B, the local manuscript handoff](prompts/PHASE_5B_LOCAL_MANUSCRIPT.md)** from the [new prompt index](CURSOR_IMPLEMENTATION_PROMPTS.md) with this plan and the code plan available to local Codex. It must not be sent to the NERSC code agent as an instruction to edit the paper. First reconcile the local parent and paper branch/commit identities, confirm that the local `manuscript/` checkout is initialized, and preserve in-progress author changes. Make structural, notation and source-link edits and the comment ledger before accepted numerical products arrive. Keep numerical claims explicitly pending rather than importing old results into the new ensemble narrative. Use fiducial ID 0, the shared sample-table ID and the validated covariance/mask when checking the compact summaries and selected publication outputs supplied by remote Cursor.

After E0–E5 products pass their gates, transfer and hash-check the export bundle, choose the optional ensemble-distribution visualization from the measured spread and request any required figure regeneration from the remote code owner. Integrate figures and update numerical prose locally, verify bibliography/availability, then compile and inspect every page locally. Return a paper source diff, comment-response ledger, figure/summary provenance, environment/run evidence references and a clear list of unresolved scientific choices. Report the actual code-result and paper identities; update the parent gitlink only after the paper commit is available on its remote. Do not claim production runs, optimized grids, MPI scaling, parallel HDF5 throughput, arbitrary-function analytic integration or noisy-data reduced-chi-square acceptance that the evidence does not establish.

**Historical review scope (15/19 September):** the original audit covered scientific TeX sections, main document/bibliography, figure manifest and source feedback, plus code/configuration/notebook/runtime contracts. The 19 September updates reread the GitHub implementation, verified the local submodule structure and finalised these plans; this planning work did not edit scientific source or TeX, access Perlmutter runtime products, submit jobs, measure new ensemble accuracy or perform an emulator/posterior study. The submodule conversion was completed separately by the author. Historical primary references remain evidence citations; any time-sensitive publication/release assertions and NERSC installation instructions must be rechecked at implementation.
