...
Figure notebooks consume the accepted saved HDF5 spectra/timings and metadata; they do not rerun the 1,001-case calculations. The saved-spectrum name and schema follow the code plan, including explicit NUMERIC interpolation identity: for example `Spectra_Triple_128_EE.h5` and `Spectra_Triple_128_LINEAR_EE.h5` within their method/device/survey/run directories. HDF5 `.h5` replaces the earlier proposed spectrum `.npz` products; the separate canonical `Cosmologies.npz` table can retain its format. The `128` filename token denotes the existing host-CPU allocation convention, including for GPU runs, and is not a GPU count. Preserve the existing timing-text naming conventions, adding NUMERIC interpolation identity as specified in the code plan. Distinguish NUMERIC integration timing from isolated analytic contraction timing and avoid promising a production run duration before pilot measurements.
