...
**Finalised 19 September 2026 — implementation plan, not an implemented manuscript revision.** Read alongside [COAUTHOR_COMMENT_INVENTORY.md](COAUTHOR_COMMENT_INVENTORY.md), [CODE_REVISION_PLAN.md](CODE_REVISION_PLAN.md), and [CURSOR_IMPLEMENTATION_PROMPTS.md](CURSOR_IMPLEMENTATION_PROMPTS.md). This update consolidates the author's decisions on the environment, manuscript submodule, local manuscript editing and remote Perlmutter implementation, HDF5 spectra, fiducial-plus-ensemble experiment, serial benchmark execution, absolute error plots, and the scope of interpolation/grid studies. All 102 comment IDs and their individual dispositions are retained. Existing line-linked scientific source locations refer to GitHub commit [`7d29b2f`](https://github.com/CosmoCloudZhang/LimberCloud/tree/7d29b2f0a5e4f216bd4dd9b4cd36b9ffb6a4487c), the 15 September audit baseline; historical local checkout references in supporting audits are historical evidence, not descriptions of the present checkout.

The scientific implementation was reread from a fresh GitHub checkout at [`0876bf4`](https://github.com/CosmoCloudZhang/LimberCloud/tree/0876bf4a50e869be1289a3eecb46931e7c8eb534) on 19 September. It still has 24 benchmark drivers running 1,000 serial, unseeded cosmology evaluations and saving cumulative timings. Numba Double/Triple use a narrower sampling range than the other drivers. The new ensemble and saved-spectrum products below are proposed changes, not existing results. Perlmutter's checkout, environment and external arrays still require reconciliation before implementation.

The verified parent revision before this documentation update is `7652a9f0b10976c9c2527a4244e8422c9455a343`, with `manuscript/` an optional submodule pinned to paper commit `90d12f4f3e574a67c25944d27d7ded553e09402b` in `LimberCloudPaper`. The earlier `b16dcdc5486eed921dda447168fe2761ff2d2514` records the completed submodule conversion; it is historical context, not the latest planning baseline. These identities establish provenance for the plan, not the future results version. Project guides belong under `documents/`; the review package remains at `revisions/2026-09/`. The manuscript is checked out and edited locally. NERSC intentionally leaves the submodule uninitialized, with its working directory absent or empty, while the parent continues to track its commit reference. Preserve historical source links and use the paper repository for actual local edits.
